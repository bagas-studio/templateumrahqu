...
export async function getPackageBySlug(
  ...
  // Query 1: Fetch single package record by slug
  ...
  // Query 2: Fallback — try matching by package id if slug not found
  let finalData = packageData;
  if (!finalData) {
    const { data: fallbackData } = (await supabase
      ...
      .eq("tenant_id", tenantId)
      .single()) as { data?: Record<string, unknown> | null; error?: unknown };
    finalData = fallbackData ?? null;
  }

  if (packageError && !finalData) {
    return null;
  }

  const raw = finalData as Record<string, unknown>;
  ...
  // Query 3: Fetch media gallery for this package
